...
export default function Hero() {
    ...
    const downloadPunAsImage = async () => {
        if (punRef.current) {
            const canvas = await html2canvas(punRef.current);
            const image = canvas.toDataURL("image/png");
            const link = document.createElement('a');
            link.href = image;
            link.download = 'pun.png';
            link.click();
        }
    };
    ...
    const shareOnTwitter = async () => {
        if (punRef.current) {
            const canvas = await html2canvas(punRef.current);
            const image = canvas.toDataURL("image/png");
            const twitterUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent('Check out this pun!')}&url=${encodeURIComponent(image)}`;
    ...
    return (
        <div className="flex flex-col relative justify-center items-center h-screen md:py-10 py-6 overflow-wrap max-w-3xl px-5">
            ...
            <div ref={punRef} className="mx-[2rem] relative w-full flex items-center justify-center aspect-[10/5] min-h-[200px] border rounded-lg p-4 mb-4 bg-white">
                {punLoading ? (
                    <Spinner />
                ) : punLoadingError ? (
