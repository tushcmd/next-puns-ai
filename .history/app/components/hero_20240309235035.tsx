...
export default function Hero() {
    const [messages, setMessages] = useState(""); // Declare the messages variable
    ...
    async function handleSubmit(e: FormEvent<HTMLFormElement>) {
        ...
        const messages = formData.get("messages")?.toString().trim();
        ...
        if (messages) {
            try {
                ...
                const response = await fetch(
                    "/api/pun?messages=" + encodeURIComponent(messages)
                );
        ...
        <div className="flex flex-col relative justify-center items-center h-screen md:py-10 py-6 overflow-wrap max-w-3xl px-5">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col mb-32 text-center items-center">
                <div className="mb-4">
                    ...
                    <input
                        value={messages}
                        onChange={(e) => setMessages(e.target.value)} // Update the messages value
